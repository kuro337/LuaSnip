...
c = {
  set_config = function(user_config)
    ...
    user_config.ext_opts = ext_util.child_complete(user_config.ext_opts or {})
    ...
    user_config.update_events = user_config.update_events or user_config.updateevents
  ...
  _setup = function()
    ...
    if session.config.delete_check_events ~= nil then
      ls_autocmd(session.config.delete_check_events, require('luasnip').unlink_current_if_deleted)
    end
    ls_autocmd(session.config.update_events, require('luasnip').active_update_dependents)
    ...
    ls_autocmd({ 'BufDelete', 'BufWipeout' }, function(event)
      ...
      if current_nodes then current_nodes[event.buf] = nil end
    end)
    if session.config.enable_autosnippets then
      ls_autocmd('InsertCharPre', function() Luasnip_just_inserted = true end)
