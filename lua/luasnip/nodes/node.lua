...
function Node:get_static_text()
  ...
  if not self.visible and not self.static_visible then return nil end
...
function Node:jump_into(_, no_move, dry_run)
  if not dry_run then self:input_enter(no_move, dry_run) end
  return self
end
...
function Node:set_old_text() self.old_text = self:get_text() end
...
local function find_dependents(self, position_self, dict)
  ...
  if self.key then vim.list_extend(nodes, dict:find_all({ 'key', self.key, 'dependents' }, 'dependent') or {}) end

  return nodes
end

function Node:_update_dependents()
  local dependent_nodes = find_dependents(self, self.absolute_insert_position, self.parent.snippet.dependents_dict)
  if #dependent_nodes == 0 then return end
  for _, node in ipairs(dependent_nodes) do
    if node.visible then node:update() end
  end
end
...
function Node:_update_dependents_static()
  ...
  if #dependent_nodes == 0 then return end
  for _, node in ipairs(dependent_nodes) do
    if node.static_visible then node:update_static() end
  end
end
...
function Node:expand_tabs(tabwidth, indentstr) util.expand_tabs(self.static_text, tabwidth, indentstr) end

function Node:indent(indentstr) util.indent(self.static_text, indentstr) end

function Node:subsnip_init() end

function Node:init_positions(position_so_far) self.absolute_position = vim.deepcopy(position_so_far) end

function Node:init_insert_positions(position_so_far) self.absolute_insert_position = vim.deepcopy(position_so_far) end

function Node:event(event)
  local node_callback = self.node_callbacks[event]
  if node_callback then node_callback(self) end
  ...
  if self.pos then
    ...
    if parent_callback then parent_callback(self) end
  end
...
local function get_args(node, get_text_func_name)
  ...
  for _, arg in ipairs(node.args_absolute) do
    ...
    if not argnode_text then return nil end
...
function Node:get_jump_index() return self.pos end
...
function Node:set_ext_opts(name)
  ...
  if name == 'passive' then
    ...
    --- regularly causes the crash as
    self.mark:update_opts(self.ext_opts[name])
  end
end

-- for insert,functionNode.
function Node:store() self.static_text = self:get_text() end
...
function Node:set_argnodes(dict)
  ...
  if self.key then dict:set({ 'key', self.key, 'node' }, self) end
end

function Node:make_args_absolute() end

function Node:resolve_position(position)
  error(string.format('invalid resolve_position(%d) on node at %s', position, vim.inspect(self.absolute_position)))
end

function Node:static_init() self.static_visible = true end
...
function Node:resolve_node_ext_opts(base_prio, parent_ext_opts)
  ...
  ext_util.set_abs_prio(self.ext_opts, (base_prio or self.parent.ext_opts.base_prio) + session.config.ext_prio_increase)
end
...
function Node:is_active(dry_run) return (not dry_run and self.active) or (dry_run and dry_run.active[self]) end
...
local function focus_node(self, lrgrav, rrgrav)
  ...
  for _, direction in ipairs({ -1, 1 }) do
    ...
    for i = 1, #nodes_path do
      ...
      if node.snippet == node then effective_direction_rgrav = direction == 1 end
      ...
      if node_above then
        node_above:set_sibling_rgravs(node, self_direction_endpoint, direction, effective_direction_rgrav)
      end
    end
...
function Node:subtree_set_rgrav(rgrav) self.mark:set_rgravs(rgrav, rgrav) end

function Node:subtree_set_pos_rgrav(_, direction, rgrav) self.mark:set_rgrav(-direction, rgrav) end

function Node:focus() focus_node(self, false, true) end

function Node:set_text(text)
  self:focus()

  local node_from, node_to = self.mark:pos_begin_end_raw()
  local ok = pcall(vim.api.nvim_buf_set_text, 0, node_from[1], node_from[2], node_to[1], node_to[2], text)
  -- we can assume that (part of) the snippet was deleted; remove it from
  -- the jumplist.
  if not ok then error('[LuaSnip Failed]: ' .. vim.inspect(text)) end
end
...
function Node:extmarks_valid() return true end
