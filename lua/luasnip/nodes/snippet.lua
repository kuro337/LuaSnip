...
local generate_resolve_expand_params_func = function(condition, user_resolve)
  return function(self, line_to_cursor, match, captures)
    if condition then
      if not condition(line_to_cursor, match, captures) then return nil end
    end
...
local function P(indx) return Parent_indexer:new({ indx = indx }) end
...
local function init_snippetNode_opts(opts)
  ...
  in_node.child_ext_opts = ext_util.child_complete(vim.deepcopy(opts.child_ext_opts or {}))
...
local function init_snippet_context(context, opts)
  ...
  if context.docstring ~= nil then effective_context.docstring = util.to_line_table(context.docstring) end
  ...
  effective_context.resolveExpandParams =
    generate_resolve_expand_params_func(context.condition or opts.condition, context.resolveExpandParams)
...
local function _S(snip, nodes, opts)
  ...
  function snip:_update_dependents()
    if self.parent_node then self.parent_node:update_dependents() end
  end
...
extend_decorator.register(S, { arg_indx = 1, extend = node_util.snippet_extend_context }, { arg_indx = 3 })
...
local function ISN(pos, nodes, indent_text, opts)
    ...
    if vim.bo.expandtab then
      ...
      for str in vim.gsplit(indent_text, '$PARENT_INDENT', true) do
        ...
        indentstring = indentstring
          .. util.expand_tabs({ str }, util.tab_width(), #indentstring + #parent_indent)[1]
          .. parent_indent
      end
  ...
  function snip:indent(parent_indent) Snippet.indent(self, get_indent(parent_indent)) end
  ...
  function snip:expand_tabs(tabwidth, indentstrlen)
    Snippet.expand_tabs(self, tabwidth, #get_indent(string.rep(' ', indentstrlen)))
  end
...
function Snippet:remove_from_jumplist()
  ...
  local sibling_list = self.parent_node ~= nil and self.parent_node.parent.snippet.child_snippets
    or session.snippet_roots[vim.api.nvim_get_current_buf()]
  local self_indx
  for i, snip in ipairs(sibling_list) do
    if snip == self then self_indx = i end
  end
...
local function insert_into_jumplist(snippet, start_node, current_node, parent_node, sibling_snippets, own_indx)
  ...
  local prev, next
  if prev_snippet ~= nil and prev_snippet.parent_node == parent_node then prev = prev_snippet end
  if next_snippet ~= nil and next_snippet.parent_node == parent_node then next = next_snippet end
  ...
  if parent_node then
    if node_util.linkable_node(parent_node) then
      ...
      if link_children and next ~= nil then
        ...
        if link_children then parent_node.inner_last = snippet.insert_nodes[0] end
...
function Snippet:trigger_expand(current_node, pos_id, env, indent_nodes)
  ...
  if current_node then
    if parent_node then
      ...
      if n_siblings_pre ~= #sibling_snippets then
        ...
        _, _, own_indx, _ = node_util.snippettree_find_undamaged_node(pos, {
          tree_respect_rgravs = false,
          tree_preference = node_util.binarysearch_preference.outside,
          snippet_mode = 'linkable',
        })
  ...
  local pre_expand_res = self:event(events.pre_expand, { expand_pos = pos }) or {}
  ...
  if indent_nodes then
    ...
    if vim.bo.expandtab then self:expand_tabs(util.tab_width(), #indentstring) end
    self:indent(indentstring)
  end

  -- (possibly) keep user-set opts.
  if self.merge_child_ext_opts then
    self.effective_child_ext_opts = ext_util.child_extend(vim.deepcopy(self.child_ext_opts), session.config.ext_opts)
  else
  ...
  self:resolve_node_ext_opts(parent_ext_base_prio, self.effective_child_ext_opts[self.type])
...
function Snippet:matches(line_to_cursor)
  ...
  local expand_params = self:resolveExpandParams(line_to_cursor, match, captures)
  if not expand_params then return nil end
  ...
  if
    self.wordTrig
    and not (from == 1 or string.match(string.sub(line_to_cursor, from - 1, from - 1), '[%w_]') == nil)
  then
    return nil
  end
...
function Snippet:copy() return copy3(self) end
...
function Snippet:is_interactive(info)
  for _, node in ipairs(self.nodes) do
    ...
    if node:is_interactive(info) then return true end
  end
  return false
end
...
function Snippet:fake_expand(opts)
  ...
  setmetatable(self.captures, {
    __index = function(_, key) return '$CAPTURE' .. tostring(key) end,
  })
...
function Snippet:get_static_text()
  ...
  if not self.static_visible then return nil end
  local text = { '' }
  for _, node in ipairs(self.nodes) do
...
function Snippet:subsnip_init() node_util.subsnip_init_children(self, self.nodes) end

Snippet.init_positions = node_util.init_child_positions_func('absolute_position', 'nodes', 'init_positions')
Snippet.init_insert_positions =
  node_util.init_child_positions_func('absolute_insert_position', 'insert_nodes', 'init_insert_positions')
...
function Snippet:text_only()
  for _, node in ipairs(self.nodes) do
    if node.type ~= types.textNode then return false end
  end
  return true
end
...
function Snippet:event(event, event_args)
  ...
  if m1_cb then m1_cb_res = m1_cb(self, event_args) end

  cb = self.node_callbacks[event]
  if cb then cb_res = cb(self, event_args) end
  ...
  if self.type == types.snippetNode and self.pos then
    ...
    if parent_cb then parent_cb_res = parent_cb(self) end
  end
...
function Snippet:find_node(predicate)
  for _, node in ipairs(self.nodes) do
    if predicate(node) then
      ...
      if node_in_child then return node_in_child end
    end
  end
  return nil
end
...
function Snippet:insert_to_node_absolute(position)
  if #position == 0 then return self.absolute_position end
...
function Snippet:resolve_position(position)
  ...
  if position == -1 and self.type == types.snippet then return self.prev end
...
function Snippet:resolve_child_ext_opts()
  if self.merge_child_ext_opts then
    self.effective_child_ext_opts =
      ext_util.child_extend(vim.deepcopy(self.child_ext_opts), self.parent.effective_child_ext_opts)
  else
    self.effective_child_ext_opts = vim.deepcopy(self.child_ext_opts)
  end
end

local function no_match() return nil end
...
function Snippet:invalidate()
  ...
  snippet_collection.invalidated_count = snippet_collection.invalidated_count + 1
end

-- used in add_snippets to get variants of snippet.
function Snippet:retrieve_all() return { self } end
...
function Snippet:subtree_set_pos_rgrav(pos, direction, rgrav)
  ...
  node_util.nodelist_adjust_rgravs(self.nodes, child_from_indx, pos, direction, rgrav, true)
end
...
function Snippet:node_at(pos, mode)
  ...
  self:find_node(function(node)
    ...
    if not node_from or not node_to then
      local error_msg = string.format(
        'LUASNIP [snippet.lua:1465] ERROR in self:find_node:\nNODE: %s\nNODE_MARK: %s\n',
        vim.inspect(node),
        vim.inspect(node_mark)
      )
      print(error_msg)
      return false
    end
    ...
    if util.pos_cmp(pos, { node_to[1], node_to[2] + 1 }) > 0 then return false end
    ...
    if node_mark:get_rgrav(-1) then grav_adjusted_from[2] = grav_adjusted_from[2] + 1 end
    if node_mark:get_rgrav(1) then grav_adjusted_to[2] = grav_adjusted_to[2] + 1 end
    ...
    if cmp_grav_from >= 0 and cmp_grav_to < 0 then table.insert(gravity_matches, node) end
    -- matches does not have to respect the extmark-conventions, just catch
    -- all possible nodes.
    if cmp_pos_from >= 0 and cmp_pos_to <= 0 then table.insert(matches, node) end
  end)
  ...
  return (function()
    for _, node in ipairs(gravity_matches) do
      if node[mode](node) then return node end
    end
