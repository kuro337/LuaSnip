...
local function init_node_opts(opts)
  ...
  in_node.node_ext_opts = ext_util.complete(vim.deepcopy(opts.node_ext_opts or {}))
...
local function snippet_extend_context(arg, extend)
  if type(arg) == 'string' then arg = { trig = arg } end
...
local binarysearch_preference = {
  outside = function(cmp_mid_to, cmp_mid_from, _) return cmp_mid_to >= 0, cmp_mid_from <= 0 end,
  inside = function(cmp_mid_to, cmp_mid_from, _) return cmp_mid_to > 0, cmp_mid_from < 0 end,
...
local function binarysearch_pos(nodes, pos, respect_rgravs, boundary_resolve_mode)
  ...
  if #nodes == 0 then return nil, 1 end
  while true do
    ...
    if respect_rgravs then
      ...
      if mid_mark:get_rgrav(-1) then mid_from[2] = mid_from[2] + 1 end
      if mid_mark:get_rgrav(1) then mid_to[2] = mid_to[2] + 1 end
    end
    ...
    if cont_behind_mid then
      ...
      left = mid + 1
      if left > right then return nil, mid + 1 end
    elseif cont_before_mid then
      -- continue search on left side
      right = mid - 1
      if left > right then return nil, mid end
    else
...
local function first_common_snippet_ancestor_path(a, b)
  ...
  if other == nil then return nil, a_path, b_path end
...
local function refocus(from, to)
  ...
  local first_common_snippet, from_snip_path, to_snip_path =
    first_common_snippet_ancestor_path(from and from.parent.snippet, to and to.parent.snippet)
  ...
  if #from_snip_path > 0 then
    ...
    if from.type == types.exitNode then
      ...
      -- needs to be called.
      ok2 = pcall(from.parent.snippet.input_leave, from.parent.snippet, true)
    end
    if not ok1 or not ok2 then from.parent.snippet:remove_from_jumplist() end
  end
  for i = 2, #from_snip_path do
    ...
    if node.type == types.exitNode then
      ...
      ok3 = pcall(node.parent.snippet.input_leave, node.parent.snippet, true)
    end

    if not ok1 or not ok2 or not ok3 then from.parent.snippet:remove_from_jumplist() end
  end
  ...
  if common_node and final_leave_node then
    ...
    if final_leave_node ~= from then final_leave_node:input_leave_children() end
  ...
  if common_node and first_enter_node then
    ...
    if first_enter_node ~= to then first_enter_node:input_enter_children() end
  end
...
local function snippettree_find_undamaged_node(pos, opts)
  ...
  while true do
    ...
    found_parent, child_indx =
      binarysearch_pos(prev_parent_children, pos, opts.tree_respect_rgravs, opts.tree_preference)
