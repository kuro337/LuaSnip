...
local function get_current_line_to_cursor() return line_chars_before(get_cursor_0ind()) end
...
local function indent(text, indentstring)
  for i = 2, #text - 1, 1 do
    ...
    if #text[i] > 0 then text[i] = indentstring .. text[i] end
  end
  ...
  if #text > 1 then text[#text] = indentstring .. text[#text] end
  return text
end
  ...
  for i, line in ipairs(text) do
    ...
    while true do
      ...
      if tab_indx then
        ...
        new_line = new_line .. string.rep(' ', tabwidth - ((parent_indent_displaycolumns + #new_line) % tabwidth))
      else
...
local function tab_width() return vim.bo.shiftwidth ~= 0 and vim.bo.shiftwidth or vim.bo.tabstop end
...
local function move_to_mark(id)
  local new_cur_pos
  new_cur_pos = vim.api.nvim_buf_get_extmark_by_id(0, session.ns_id, id, { details = false })
...
local function multiline_equal(t1, t2)
  for i, line in ipairs(t1) do
    if line ~= t2[i] then return false end
  end
...
local function string_wrap(lines, pos)
  ...
  if #new_lines == 1 and #new_lines[1] == 0 then return { '$' .. (pos and tostring(pos) or '{}') } end
...
local function pos_add(p1, p2) return { p1[1] + p2[1], p1[2] + p2[2] } end
local function pos_sub(p1, p2) return { p1[1] - p2[1], p1[2] - p2[2] } end
...
local function no_region_check_wrap(fn, ...)
  ...
  vim.schedule(function() session.jump_active = false end)
  return fn(...)
end
